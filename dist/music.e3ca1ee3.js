...
let hogwartsPlaylist = new playlist("Hogwarts Ambience", [
    "Hedwig's Theme.mp3"
]);
let bardcorePlaylist = new playlist("Bardcore", [
    "Never Gonna Give You Up (Medieval Cover).mp3"
]);
let studioGhibli = new playlist("Studio Ghibli OST", [
    "A town with an ocean view.mp3"
]);
let witcherAmbience = new playlist("The Witcher III", [
    "Kaer Morhen.mp3"
]);
let rain = new playlist("Rain Sounds", [
    "rain sounds.mp3"
]);
let lofi = new playlist("Lofi", [
    "Forest Lullaby.mp3"
]);
let fireplace = new playlist("Fireplace", [
    "Fireplace.mp3"
]);
let playlists = [
    hogwartsPlaylist,
...
function loadPlaylist(playlistName) {
    ...
    for(let y = 0; y < playlists.length; y++)if (playlistName.innerText == playlists[y].playlistName) {
        let songToPlay = playlists[y].tracklist[0];
        playSong(songToPlay, playlistStatusIcon);
    }
}
for(let x = 0; x < playlistCovers.length; x++)playlistCovers[x].addEventListener("click", ()=>{
    ...
    if (isPlaying) pauseSong(playlistName.firstElementChild);
    else loadPlaylist(playlistName);
});
function playSong(songToPlay, statusIcon1) {
    audio.classList.add("playing");
    statusIcon1.classList.replace("playButtonActive", "pauseButtonActive");
    statusIcon1.src = "icons/pause.svg";
    audio.src = "songs/" + songToPlay;
    audio.src = songToPlay;
    console.log("song to play is " + audio.src);
    //audio.load();
    var playPromise = audio.play();
    if (playPromise !== undefined) playPromise.then(function() {
    ...
    function pauseSong(statusIcon2) {
        audio.classList.remove("playing");
        statusIcon2.classList.replace("pauseButtonActive", "playButtonActive");
        statusIcon2.src = "icons/play.svg";
        audio.pause();
...
//# sourceMappingURL=music.e3ca1ee3.js.map
