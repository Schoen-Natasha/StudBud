...
function findSynonyms(wordInput) {
    ...
    for(let j = 0; j < wordInput[0].meanings.length; j++){
        ...
        for(let i = 0; i < synonymArrayLocation.length - 1; i++)if (synonymArrayLocation[i] == "synonyms" && wordInput[0].meanings[j].synonyms.length > 0) {
...
function resetSearchContainer(searchResultContainer1) {
    //removes all child nodes of the search container
    while(searchResultContainer1.firstChild)searchResultContainer1.removeChild(searchResultContainer1.firstChild);
}
